...
try{
 if($a==='settings'){$rows=$pdo->query('SELECT setting_key,setting_value FROM system_settings')->fetchAll(PDO::FETCH_KEY_PAIR);$out=[];foreach($rows as $k=>$val){$decoded=json_decode($val,true);$out[$k]=json_last_error()===JSON_ERROR_NONE?$decoded:$val;}echo json_encode(['ok'=>true,'settings'=>$out],JSON_UNESCAPED_UNICODE);exit;}
